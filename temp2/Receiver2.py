...
# Packet size for receiving data
INCOMING_PACKET_SIZE = 1024
# Header format: flag (1 byte) and sequence number (1 byte)
INCOMING_HEADER_FORMAT = "!BB"
BYTES_PER_HEADER = struct.calcsize(INCOMING_HEADER_FORMAT)

# ACK format: one unsigned byte for the sequence number
ACK_FORMAT = "!B"
...
def receive_file_over_rdt3(filename, listen_port):
    ...
    print("Receiver2 listening on {listen_ip}:{listen_port}...")
    expected_seq = 0
    with open(filename, "wb") as outfile:
        while True:
            ...
            if flag == 0:  # current packet holds data
                ...
                # Send an ACK for the EOF packet and break the loop
